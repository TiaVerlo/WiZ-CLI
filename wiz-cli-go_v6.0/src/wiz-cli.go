...
func printHelp() {
    ...
    fmt.Printf("  %s-file%s <path>                 - load bulb IPs from file\n", FgYellow, Reset)
    fmt.Printf("  %s-ips%s <ip1,ip2,...>           - specify comma-separated bulb IPs\n", FgYellow, Reset)
    fmt.Printf("%s------------------------------------------------------------------------------------%s\n", FgYellow, Reset)
}
...
func main() {
    ...
    if exePath, err := os.Executable(); err == nil {
        ...
        if _, err := os.Stat(appBundleResPath); err == nil {
            loadScenesFromFile(appBundleResPath)
        } else if _, err := os.Stat(localPath); err == nil {
            loadScenesFromFile(localPath)
        }
    }
    ...
    var initialIPs []string
    if bulbFile != "" {
        if ipsFromFile, err := loadIPsFromFile(bulbFile); err == nil {
            initialIPs = append(initialIPs, ipsFromFile...)
            fmt.Printf("loaded %d bulb IP(s) from file: %s\n", len(ipsFromFile), bulbFile)
        } else {
            fmt.Printf("%s%scould not load bulb file %s: %v%s\n", ErrorPrefix, FgRed, bulbFile, err, Reset)
        }
    }
    if ipFlag != "" {
        parts := strings.Split(ipFlag, ",")
        for _, p := range parts {
            p = strings.TrimSpace(p)
            if p != "" {
                initialIPs = append(initialIPs, p)
            }
        }
    }
    ...
    if len(initialIPs) > 0 {
        ...
        ips := discoverBulbs(4 * time.Second)
        for i, ip := range ips {
            manager.bulbs = append(manager.bulbs, NewBulb(i+1, ip, udpHandler, sceneRunner))
        }
    }
    ...
    if len(cliArgs) > 0 {
        time.Sleep(200 * time.Millisecond)
        executeCommand(manager, cliArgs)
        if sceneRunner.hasActiveTasks() {
            select {}
        } else {
    ...
    for {
        ...
        if len(fields) > 0 {
            if executeCommand(manager, fields) {
                break
            }
        }
    }
